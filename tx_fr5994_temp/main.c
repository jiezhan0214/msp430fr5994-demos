...
int main(void) {
    ...
    // Flush TX FIFO
    status = nrf24Cmd(RF24_FLUSH_TX);

    // clear power-up data sent flag, write 1 to clear
    // 0x0E default
    status = nrf24RegWrite(RF24_STATUS, RF24_TX_DS);

    // clear PWR_UP, 0x08 default CONFIG state
    status = nrf24RegWrite(RF24_CONFIG, 0x08);

    // Read the STATUS and CONFIG registers
    status = nrf24RegRead(RF24_CONFIG, &read_byte);
#ifdef ASSERT
    ...
    status = nrf24RegWrite(RF24_CONFIG, RF24_EN_CRC | RF24_PWR_UP);
#ifdef ASSERT
    ...
    // Disable retransmit;
    status = nrf24RegWrite(RF24_EN_AA, 0x00);  // Clear all auto acks
#ifdef ASSERT
    assert(status == 0b00001110);
#endif

    status = nrf24RegWrite(RF24_EN_RXADDR, 0x01);  // Enable Rx data pipe 0 only
#ifdef ASSERT
    assert(status == 0b00001110);
#endif

    status = nrf24RegWrite(RF24_SETUP_RETR, 0x00);  // Disable retransmit
#ifdef ASSERT
    assert(status == 0b00001110);
#endif
...
#endif
    status = nrf24RegRead(RF24_RF_CH, &read_byte);
#ifdef ASSERT
    assert(status == 0x0E);
    ...
    for (;;) {
...
#ifdef ASSERT
        status = nrf24RegRead(RF24_FIFO_STATUS, &read_byte);
        assert(status == 0x0E);
        ...
        // 1 level
        status = nrf24WritePayload(dummy_payload, 32);
#ifdef ASSERT
        assert(status == 0b00001110);
        status = nrf24RegRead(RF24_FIFO_STATUS, &read_byte);
        assert(status == 0x0E);
        ...
        // Power down
        // status = nrf24RegWrite(RF24_CONFIG, RF24_EN_CRC);
